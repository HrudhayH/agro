...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <motion.div className="group bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-shadow duration-500 border border-earth-50 flex flex-col h-full" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, type: "spring", stiffness: 100 }} whileHover={{ y: -6 }}>
      <div className="relative aspect-square overflow-hidden bg-rice-100">
        ...
        <button className="absolute top-4 right-4 z-10 p-2.5 rounded-full bg-white/80 backdrop-blur-sm text-earth-300 hover:text-rose-500 hover:bg-white transition-all shadow-md opacity-0 group-hover:opacity-100" aria-label="Add to Wishlist">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-10">
          <button className="w-full bg-earth-500 hover:bg-earth-400 text-white py-3 rounded-2xl font-semibold flex items-center justify-center gap-2 shadow-xl transition-colors">
