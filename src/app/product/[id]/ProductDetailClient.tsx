  ...
  return (
    <>
      ...
      <main className="bg-background min-h-screen">
        ...
        <section className="py-10 md:py-16 bg-rice-100">
          <div className="container mx-auto px-4 md:px-8 max-w-7xl">
            <div className="flex flex-col lg:flex-row gap-10 lg:gap-16 items-start">
              ...
              >
                ...
                {/* Title & Tagline */}
                <div>
                  <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl font-bold text-earth-600 mb-3 leading-tight">{product.name}</h1>
                  <p className="text-mango-600 font-semibold text-base md:text-lg italic">{product.tagline}</p>
                </div>

                {/* Short Description */}
                <p className="text-earth-400 text-base md:text-lg leading-relaxed border-l-4 border-mango-300 pl-5 py-2">{product.description}</p>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {product.variants.map((v, idx) => (
                      <button
                        ...
                        className={`relative px-4 md:px-5 py-3 rounded-2xl border-2 text-sm font-semibold transition-all duration-300 flex flex-col items-start ${
                ...
                <div className="flex items-baseline gap-4">
                  <span className="font-serif text-4xl md:text-5xl font-bold text-earth-600">Rs.{variant.price}</span>
                  {variant.originalPrice && (
                    <>
                      <span className="text-lg md:text-xl text-earth-300 line-through">Rs.{variant.originalPrice}</span>
                      <span className="bg-accent/10 text-accent font-bold text-sm px-3 py-1 rounded-full">{discount}% OFF</span>
                    </>
                  )}
                  <span className="text-sm text-earth-400 ml-auto">{variant.weight}</span>
                </div>
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  ...
                  {/* Add to Cart */}
                  <button className="flex-1 min-w-[180px] group flex items-center justify-center gap-3 bg-earth-600 hover:bg-earth-500 text-white font-bold px-6 md:px-8 py-4 rounded-full transition-all hover:scale-[1.02] shadow-xl shadow-earth-500/20 text-base md:text-lg">
                ...
                <div className="flex flex-wrap gap-2 md:gap-3 pt-2">
                  {["Farm-Direct", "No Artificial Ripeners", "COD Available", "Free Shipping on Rs.999+"].map((tag) => (
                    <span key={tag} className="flex items-center gap-2 text-xs md:text-sm text-earth-500 bg-earth-50 border border-earth-100 px-3 md:px-4 py-2 rounded-full font-medium">
                      <CheckCircle2 className="w-4 h-4 text-mango-500 shrink-0" /> {tag}
                    </span>
                  ))}
                </div>
              </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
              ...
              <div>
                ...
                >
                  ...
                  <h2 className="font-serif text-3xl md:text-4xl font-bold text-earth-600 mb-8 md:mb-10">Health Benefits</h2>
                </motion.div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-5">
                  {product.healthBenefits.map((benefit, idx) => (
                    <motion.div
                      key={idx}
                      className="p-5 md:p-6 rounded-2xl bg-earth-50 border border-earth-100 hover:shadow-md hover:-translate-y-1 transition-all"
                      initial={{ opacity: 0, y: 20 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      viewport={{ once: true }}
              ...
              <div>
                ...
                >
                  ...
                  <h2 className="font-serif text-3xl md:text-4xl font-bold text-earth-600 mb-8 md:mb-10">100% Organic & Natural</h2>
                </motion.div>
                <div className="flex flex-col gap-4 md:gap-5">
                  {product.organicPoints.map((point, idx) => (
                    <motion.div
                      key={idx}
                      className="flex gap-4 md:gap-5 p-5 md:p-6 rounded-2xl bg-white border border-earth-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all"
                      initial={{ opacity: 0, x: 30 }}
